...
class darcn(nn.Module):
    ...
    @classmethod
    def load_model(cls, path, causal_flag, stage_number):
        package = torch.load(path, map_location=lambda storage, loc: storage)
        model = cls.load_model_from_package(package, causal_flag, stage_number)
        return model

    @classmethod
    def load_model_from_package(cls, package, causal_flag, stage_number):
        model = cls(causal_flag=causal_flag, stage_number=stage_number)
...
class AEncoder(nn.Module):
    def __init__(self):
        ...
        self.en1 = nn.Sequential(
            self.pad1,
            nn.Conv2d(in_channels=2, out_channels=16, kernel_size=(2, 5), stride=(1, 2)),
            nn.BatchNorm2d(16),
            nn.ELU())
...
class MDecoder(nn.Module):
    def __init__(self):
        ...
        self.de6 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=16, out_channels=1, kernel_size=(1,1), stride=(1, 1)),
            nn.Softplus())
...
class Stage_GRU(nn.Module):
    def __init__(self):
        ...
        self.pre_conv = nn.Sequential(
            self.pad,
            nn.Conv2d(in_channels=2, out_channels=16, kernel_size=(2, 5), stride=(1, 1)),
            nn.ELU())
