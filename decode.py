...
def enhance(args):
    ...
    with torch.no_grad():
        ...
        for file_id in file_list:
            ...
            feat_wav = feat_wav / c
            feat_x = librosa.stft(feat_wav, n_fft=fft_num, hop_length=win_shift, window='hamming').T
            ...
            esti_utt = librosa.istft((de_esti).T, hop_length=win_shift,
                                     win_length=win_size, window='hamming', length= len(feat_wav)).astype(np.float32)
            esti_utt = esti_utt * c
